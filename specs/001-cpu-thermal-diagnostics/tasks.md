...
- [ ] T153 Empaquetar el instalador oficial de PawnIO con versión fijada en `apps/desktop/src-tauri/` (recurso del bundle NSIS, no descargable en ejecución), verificar su firma Authenticode y hash antes de lanzarlo, e implementar `request_low_level_access` en `src-tauri/src/commands/` (una sola UAC, solo en estados `installable`/`upgradable`/reparación) con test de integración Windows según FR-087 (implementación técnica lista; pendiente revisión legal del instalador y prueba de integración Windows)
- [ ] T154 Implementar el lanzador elevado del sidecar y el transporte IPC autenticado aprobados en ADR-0004: registro durante T153, arranque sin UAC en sesiones posteriores, ruta fija + hash, nonce, regla de vida; adaptar `src-tauri/src/ipc/supervisor.rs` para el modo elevado manteniendo el modo hijo sin privilegios; TDD en el rechazo de conexiones sin nonce y en la caída del lanzador según FR-088, Constitución IV (implementación técnica lista; pendiente prueba Windows del ciclo completo y watchdog)
