...
-  Evidencia parcial 2026-09-19: `cargo nextest run --profile ci --no-tests=pass` (0; 73/73), `cargo clippy --locked --all-targets -- -D warnings` (0), `cargo fmt --all -- --check` (0) y `cargo run --locked --bin elevated-integration -- --bootstrap` (0, antes y después del reinicio; `LastTaskResult=0`). La prueba negativa cubre comando desconocido, escritura genérica de MSR, ruta/hash incorrectos, nonce reutilizado y tubería ocupada. No se marca: falta manifiesto minisign integrado y no se pudo reproducir «servicio detenido» porque PawnIO arrancó automáticamente tras el reinicio; detenerlo en caliente devolvió 1052.
