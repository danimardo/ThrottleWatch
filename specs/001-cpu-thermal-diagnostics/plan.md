...
|---|---|---|---|---|---|---|
| A1 | XVII (aclaración de ámbito, no excepción) | `scripts/**` (utillaje de desarrollo y de agentes, ejecutado con Node fuera de la aplicación) y los generadores de activos de marca bajo `design/brand/**` (p. ej. `design/brand/tray/generate.mjs`, T068) no pertenecen a las tres capas del principio XVII y pueden escribir en consola con `console.*`; ESLint excluirá `scripts/**` de `no-console` mediante `overrides` (T135); los generadores de `design/brand/**` no entran en ningún `package.json`/lint del sistema de diseño (`design/harness`, `design/mockup`), igual que `source.html` de `app-icon`. | Los scripts y generadores no se empaquetan ni se distribuyen y no tienen acceso al envoltorio de la interfaz. | Crear un envoltorio para scripts: complejidad sin beneficio. | Ninguno para el usuario; ni los scripts ni los generadores manejan datos del usuario. | No aplica. |
| A2 | XVII y XV (aclaración de ámbito, no excepción) | `apps/desktop/src-tauri/build.rs` (script de compilación: habla con cargo por `stdout` mediante directivas `cargo:` y lee solo variables de compilación de cargo como `OUT_DIR` o `CARGO_CFG_TARGET_ENV`) y `docs/spikes/**` (bancos de medida y guiones de spikes que no se empaquetan, no se lintean y no forman parte de la solución ni de CI) no pertenecen a las tres capas del principio XVII ni a la configuración de la aplicación (XV). | Un build script no tiene envoltorio posible y los bancos de spikes son utillaje de investigación fuera de la aplicación. | Crear envoltorios o incluir los bancos en CI: complejidad sin beneficio. | Ninguno para el usuario; no manejan datos del usuario. Los `allow` de clippy en `build.rs` citan esta fila. | No aplica. |
...
| E2 | Puerta de calidad 4 (traza Intel, AMD y degradada) y metodología por lotes (`tasks.md` § Lotes: «un lote solo empieza con el `CHK` de sus predecesores cerrado») | CHK-L03 se cierra el **2026-09-19** con tres verificaciones de hardware diferidas: T019 (filas Intel híbrido, Intel anterior, portátil OEM y AMD Zen 4; contraste APERF/MPERF), T019b (corpus etiquetado real) y la mitad Intel de T019c. Las tareas quedan abiertas en `tasks.md`; L05 y T153–T155 pueden empezar. | Los equipos de la matriz no están disponibles y no existe todavía un grabador de trazas; el resto de L03 (colector real, VM de CI, PawnIO, permisos de Tauri, vidrio en el 2600X) está verificado. L03 cubre AMD (2600X) y degradada (VM de CI) pero **no** una traza Intel real. | Esperar al hardware: bloquearía toda la implementación sin aportar información nueva sobre el código ya escrito. | (1) El corpus actual es sintético (`packages/trace-fixtures/corpus/labels-v1.json`, T037 cerrada sobre él): los criterios SC-003–SC-005 y SC-016–SC-018 y la calibración de `research.md` § 15 **no pueden declararse cumplidos** hasta grabar T019b. (2) Ninguna característica que dependa del colector puede darse por terminada según la puerta 4 hasta cumplir la fila Intel de T019. (3) Los umbrales `glass.*` siguen provisionales y T131 no se cierra sin la mitad Intel de T019c. (4) *Retirado el 2026-09-19:* una versión anterior de esta fila condicionaba T154 a una enmienda de ADR-0004 por un hallazgo erróneo (sondeos tomados desde una sesión elevada); la comprobación con integridad media real confirma el resultado (b) y ADR-0004 sigue vigente sin enmienda (`sensor-access.md`, «CORRECCIÓN»). El guion `docs/spikes/tools/sensor-access-matrix.ps1` y el banco `docs/spikes/glass-bench` permiten completar la matriz sin código nuevo. | Al completar T019, T019b y T019c; fecha límite **2026-11-30**, ampliable solo mediante esta tabla. |
