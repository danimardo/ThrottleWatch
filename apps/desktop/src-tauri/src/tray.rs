...
//! `state_for` and `tray_variant` are pure (the latter modulo its WinRT read); `setup` and
//! `refresh` are the thin Tauri glue.
...
/// Which taskbar-contrast icon set to draw: `design/brand/tray`'s `dark` variant (light-toned
/// strokes) for a dark taskbar, `light` (dark-toned strokes) for a light one. The three coloured
/// states (normal/warning/critical) use the same hex in both variants; only the neutral tone of
/// `unknown`/`disconnected` changes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TrayVariant {
    Dark,
    Light,
...
/// `UISettings.GetColorValue(Background)` — the same WinRT surface `appearance.rs` already polls
/// for `AdvancedEffectsEnabled` — as an `(r, g, b)` triple. `None` when the read fails, same
/// failure shape as `appearance::advanced_effects_enabled`.
#[cfg(windows)]
fn background_color() -> Option<(u8, u8, u8)> {
...
/// Calls the background `Light` only when it is genuinely bright, `Dark` otherwise — including
/// when the read fails, since most Windows installs ship a dark taskbar by default and a wrong
/// guess there is the more common case.
#[cfg(windows)]
fn tray_variant() -> TrayVariant {
    let Some((r, g, b)) = background_color() else { return TrayVariant::Dark };
    // Perceptual luminance (ITU-R BT.601); a midpoint split is precise enough to pick between two
    // hand-drawn icon sets, not to reproduce exact taskbar contrast math.
    let luminance = 0.299 * f64::from(r) + 0.587 * f64::from(g) + 0.114 * f64::from(b);
    if luminance > 128.0 { TrayVariant::Light } else { TrayVariant::Dark }
}
...
/// The 32px PNG for a state/variant pair, generated by `design/brand/tray/generate.mjs` and
/// copied into `icons/tray/` the same way `design/brand/app-icon/icon.ico` is copied into
/// `icons/` (see `design/brand/README.md`).
fn tray_icon_bytes(state: TrayIconState, variant: TrayVariant) -> &'static [u8] {
    match (state, variant) {
        (TrayIconState::Normal, TrayVariant::Dark) => {
            include_bytes!("../icons/tray/normal-dark.png")
        }
        (TrayIconState::Normal, TrayVariant::Light) => {
            include_bytes!("../icons/tray/normal-light.png")
        }
        (TrayIconState::Warning, TrayVariant::Dark) => {
            include_bytes!("../icons/tray/warning-dark.png")
        }
        (TrayIconState::Warning, TrayVariant::Light) => {
            include_bytes!("../icons/tray/warning-light.png")
        }
        (TrayIconState::Critical, TrayVariant::Dark) => {
            include_bytes!("../icons/tray/critical-dark.png")
        }
        (TrayIconState::Critical, TrayVariant::Light) => {
            include_bytes!("../icons/tray/critical-light.png")
        }
        (TrayIconState::Unknown, TrayVariant::Dark) => {
            include_bytes!("../icons/tray/unknown-dark.png")
        }
        (TrayIconState::Unknown, TrayVariant::Light) => {
            include_bytes!("../icons/tray/unknown-light.png")
        }
        (TrayIconState::Disconnected, TrayVariant::Dark) => {
            include_bytes!("../icons/tray/disconnected-dark.png")
        }
        (TrayIconState::Disconnected, TrayVariant::Light) => {
            include_bytes!("../icons/tray/disconnected-light.png")
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::{TrayIconState, state_for};
    use crate::diagnostics::classifier::{Classification, DiagnosticResult, Severity};
    use crate::diagnostics::rules::CoverageTier;
    ...
    #[test]
    fn every_state_has_its_own_key() {
        let all = [
            TrayIconState::Normal,
            TrayIconState::Warning,
            TrayIconState::Critical,
    ...
    #[test]
    #[cfg(windows)]
    fn background_color_reads_a_real_answer_on_this_machine() {
        // Same shape as appearance::advanced_effects_enabled_reads_a_real_answer_on_this_machine:
        // a `None` here would mean the WinRT apartment/projection is broken on this build, which
        // `tray_variant` treats as "unknown, default to dark", but a passing dev/CI machine
        // should never actually hit that path.
        assert!(super::background_color().is_some());
    }

    #[test]
    fn every_state_and_variant_has_its_own_embedded_icon_and_it_decodes() {
        let states = [
            TrayIconState::Normal,
            TrayIconState::Warning,
            TrayIconState::Critical,
        ...
        let variants = [super::TrayVariant::Dark, super::TrayVariant::Light];
        for state in states {
            for variant in variants {
                let bytes = super::tray_icon_bytes(state, variant);
                assert!(
                    tauri::image::Image::from_bytes(bytes).is_ok(),
                    "{state:?}/{variant:?} icon must decode as a valid image"
                );
            }
        }
    }
